...
`mcumgr` can only be invoked in the Zephyr Python Virtual environment, so if you can't find the program, you must first install Zephyr through the getting started guide. If you don't want to do that, you can download [AuTerm](https://github.com/thedjnK/AuTerm/releases/tag/v0.36a-pre), which has a graphical interface, then select the serial port where your board is (generally /dev/ttyUSB0 when on Linux) and select speed of 115200. Then you go to the MCUmgr tab and select the file you want to flash your device with. Then you do the process of resetting the board.  
